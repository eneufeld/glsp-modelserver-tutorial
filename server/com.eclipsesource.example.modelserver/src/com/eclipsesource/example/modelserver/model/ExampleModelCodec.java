package com.eclipsesource.example.modelserver.model;
...
import com.eclipsesource.example.model.util.ModelResourceImpl;
import com.fasterxml.jackson.databind.JsonNode;

public class ExampleModelCodec implements Codec {

	private static Logger LOGGER = Logger.getLogger(ExampleModelCodec.class.getSimpleName());
	...
	public JsonNode encode(final EObject eObject) throws EncodingException {
		...
		eObject.eResource().setURI(URI.createURI("virtual.model"));
	...
	public Optional<Resource> decode(final ResourceSet resourceSet, final String modelURI, final URI workspaceURI,
		...
		if (result != null && !(result instanceof ModelResourceImpl)) {
			// Replace it
		...
		if (result == null) {
			result = resourceSet.createResource(URI.createURI(modelURI));
